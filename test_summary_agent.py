"""
Test Execution Summary Agent
Parses Serenity test execution HTML reports and generates interactive summary reports
...
class SerenityReportParser:
    ...
    def _extract_from_overview(self, metrics: dict):
        ...
        for section in summary_sections:
            ...
            if 'flaky' in text:
                match = re.search(r'(\d+)\s*flaky', text)
                if match:
                    metrics['flaky'] = int(match.group(1))
...
def generate_ai_insights(metrics: dict, html_content: str) -> dict:
    """Generate AI-powered insights from test metrics."""
    ...
    # If LLM not available, return template insights
    if not LLM_AVAILABLE:
        return _get_template_insights(metrics)
    ...
    try:
        ...
        analysis_agent = LlmAgent(
            name='TestSummaryAnalyzer',
            model=GlobalGemini(model='gemini-3.5-flash'),
            description='Agent specialized in analyzing test execution reports.',
            sub_agents=[],
            instruction=(
                'You are a QA analytics expert. Analyze the provided test execution report and provide:\n'
                '1. Root causes of failures (identify common patterns and infrastructure issues)\n'
                '2. Regressions (tests that passed before but now fail)\n'
                '3. Improvements and resolved issues\n'
                '4. Actionable recommendations\n'
                '5. Flakiness analysis\n'
                'Format response as JSON with keys: root_causes, regressions, improvements, recommendations, summary, flakiness_analysis'
            ),
            tools=[]
        )
        ...
        analysis_prompt = f"""
Analyze this test execution report and provide detailed insights:

Summary: {metrics.get('total_tests', 0)} tests, {metrics.get('passed', 0)} passed, {metrics.get('failed', 0)} failed, {metrics.get('flaky', 0)} flaky
...
- root_causes (array of strings describing failure patterns)
- regressions (array of new failures or issues)
- improvements (array of resolved issues)
- recommendations (array of actionable next steps)
- summary (string with executive summary)
- flakiness_analysis (array of flaky test patterns)
"""

        session_svc = InMemorySessionService()
        runner = Runner(session_service=session_svc, app_name='test-summary', agent=analysis_agent)
        ...
        insights = _get_template_insights(metrics)

    return insights


def _get_template_insights(metrics: dict) -> dict:
    """Return template insights when AI not available."""
...
def generate_interactive_html_report(metrics: dict, insights: dict, run_id: str = "#142") -> str:
    """Generate interactive HTML report similar to qa_genie.html report section."""
    ...
    for i, test in enumerate(metrics.get('failed_tests', [])[:10]):
        ...
        <tr style="border-bottom:1px solid var(--border);">
            <td style="padding:10px 14px;"><div style="font-weight:bold;margin-bottom:2px;">{test.get('name', 'Unknown Test')[:60]}</div><div style="font-size:9px;color:var(--text-dim);">{status_icon} {test.get('status', 'FAILED')}</div></td>
            <td style="padding:10px 14px;color:var(--red);">Test Failure</td>
            <td style="padding:10px 14px;"><span style="font-size:9px;padding:2px 7px;border-radius:3px;border:1px solid rgba(var(--red-rgb),.4);color:var(--red);background:rgba(var(--red-rgb),.08);">Failure</span></td>
            <td style="padding:10px 14px;"><span style="display:inline-flex;align-items:center;gap:5px;font-size:10px;"><span style="width:7px;height:7px;border-radius:50%;background:var(--red);display:inline-block;"></span>High</span></td>
        </tr>
        """
    ...
    # Root causes
    for i, cause in enumerate(insights.get('root_causes', [])[:3]):
        icons = ["🔴", "🟡", "🟠"]
        icon = icons[i % len(icons)]
        ai_insights_html += f"""
        <div style="display:flex;gap:10px;padding:10px 0;border-bottom:1px solid rgba(var(--purple-rgb),.12);">
            <span style="font-size:16px;flex-shrink:0;">{icon}</span>
            <div><div style="font-size:11px;font-weight:bold;margin-bottom:3px;">{cause[:70]}</div></div>
        </div>
        """

    # Improvements
    for i, improvement in enumerate(insights.get('improvements', [])[:2]):
        ai_insights_html += f"""
        <div style="display:flex;gap:10px;padding:10px 0;border-bottom:1px solid rgba(var(--purple-rgb),.12);">
            <span style="font-size:16px;flex-shrink:0;">🟢</span>
            <div><div style="font-size:11px;font-weight:bold;margin-bottom:3px;">{improvement[:70]}</div></div>
        </div>
        """
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing:border-box; margin:0; padding:0; }}
        ...
        th {{ text-align:left; padding:9px 14px; font-size:9px; color:var(--text-dim); text-transform:uppercase; font-weight:normal; }}
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card failed">
                ...
                <div class="stat-sub">{'↑ ' + str(abs(failed - prev_failed)) + ' vs last' if failed > 0 else 'All passing'}</div>
            </div>
            <div class="stat-card orange">
                <div class="stat-label">Flaky</div>
                <div class="stat-value orange">{flaky}</div>
                <div class="stat-sub">{'Unstable tests' if flaky > 0 else 'Stable'}</div>
            </div>
            <div class="stat-card orange">
                <div class="stat-label">Skipped</div>
                <div class="stat-value orange">{skipped}</div>
                <div class="stat-sub">{'Skipped' if skipped > 0 else 'None'}</div>
            </div>
            <div class="stat-card">
        ...
        <div class="two-col">
            <div class="card card-ai">
                ...
                <div style="margin-top:12px;display:flex;gap:8px;">
                    ...
                    <button style="flex:1;background:rgba(var(--accent-rgb),.15);color:var(--accent);border:1px solid rgba(var(--accent-rgb),.3);padding:8px;border-radius:6px;font-family:monospace;font-size:10px;cursor:pointer;font-weight:bold;">💾 Save Summary</button>
                </div>
            </div>

            <div class="card">
                <div class="card-title">Failure Summary</div>
                <div style="overflow-x:auto;">
                    <table>
                        ...
                        <tbody>
                            {failed_tests_html if failed_tests_html else '<tr><td colspan="4" style="text-align:center;color:var(--text-dim);">No failures - all tests passed!</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="card" style="margin-top:16px;">
            ...
            <div style="color:var(--text-3);line-height:1.8;font-size:11px;">
                <p>{insights.get('summary', 'Test execution completed successfully.')}</p>
            </div>
        </div>
    </div>
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python test_summary_agent.py <html_file_path>")
        sys.exit(1)
